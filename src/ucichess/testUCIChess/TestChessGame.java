...
public class TestChessGame {
    ...
    public void gameloop() {
        ...
        while (gameisOn) {
            ...
            System.out.println("----------------------------------------------------");
            System.out.println("== (" + count + ") WHITE PLAYER (Human) ==");
            while (!testMove) {
                whiteMove = askMove();
                testMove = test_Move(whiteMove);
                if (!testMove) {
                    System.out.println(whiteMove + " this move is not correct, must be from a1 to h8 and formated like this e2e4!");
                }
            }
  ...
  //          System.out.println("FEN=" + currFEN);
            ...
            //black move
            System.out.println("----------------------------------------------------");
            System.out.println("== (" + count + ") BLACK PLAYER (" + engName + ") ==");
...
//            System.out.println("FEN=" + currFEN);
    ...
    public boolean test_Move(String move) {
        ...
        if (move.charAt(1) < 49 || move.charAt(1) > 56) {
            return false;
        }
        if (move.charAt(2) < 97 || move.charAt(2) > 104) {
            return false;
        }
        if (move.charAt(3) < 49 || move.charAt(3) > 56) {
            return false;
        }
        ...
        if (move.length() == 5) {
            ...
            if (move.charAt(4) != 81 || move.charAt(4) != 82 || move.charAt(4) != 78 || move.charAt(4) != 66) {
                return false;
            }
        }
        ...
        if (ChessBoard.pieceIsBlack(fc, fr)) {
            return false;
        }
        if (ChessBoard.square_is_Empty(fc, fr)) {
            return false;
        }
